...
public class SplashListener implements InputProcessor{
	...
	public SplashListener(HungryFrogGame hungryfrog){
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if(posX<=217 && posX>=143 && posY<=130 && posY>=118){
			...
			hungryfrog.getMusicIntro().pause();
			hungryfrog.setSonActif(false);
			setSonActive(false);
		}
		else if(!hungryfrog.getSonActif() && posX<=133 && posX>=120 && posY<=134 && posY>=116){
			System.out.println("son-actif");
			hungryfrog.getMusicIntro().play();
			hungryfrog.setSonActif(true);
			setSonActive(true);
		}
		else if(posX<=610 && posX>=247 && posY<=130 && posY>=118){
			System.out.println("quitter");
			hungryfrog.getMusicIntro().stop();
			System.exit(0);
		}
		return false;
	}
